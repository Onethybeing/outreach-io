...
def enrich_org(db: Session, website: str | None, name: str | None = None) -> dict | None:
    """{'name', 'linkedin_url', 'website_url'} for a company, by domain if known, else by name. Never raises.
    ...
    try:
        with telemetry.observation("apollo_org_enrich", as_type="tool", input=params) as obs:
            ...
            if not domain and not names_match(result["name"], name or ""):
                return None
            return result if result["name"] or result["linkedin_url"] else None
    except (httpx.HTTPError, vault.VaultError, ValueError):
...
def find_email(db: Session, name: str, linkedin_url: str, website: str | None) -> EmailResult:
    ...
    with telemetry.observation("apollo_people_match", as_type="tool", input={"linkedin_url": linkedin_url}) as obs:
        try:
            telemetry.heartbeat()
            response = httpx.post(f"{BASE}/people/match", headers=_headers(db), json=body, timeout=30)
