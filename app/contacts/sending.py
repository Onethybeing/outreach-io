...
def build_message(contact: Contact, sender: str, resume: Resume, to_address: str, redirected: bool) -> EmailMessage:
...
def send(db: Session, contact_id: uuid.UUID, user: User, force: bool = False) -> Contact:
    ...
    if not to_address:
        raise ActionError(400, "No sending address configured — set DEV_REDIRECT_EMAIL or the Gmail sender address")
    mode = "dev" if redirected else "prod"

    contact.send_status = SendStatus.queued
    db.commit()
    try:
        message = build_message(contact, sender_address(db), resume, to_address, redirected)
    ...
    audit.record(db, user, "emails.send", "contact", contact.id, {
        "mode": mode, "to": to_address, "gmail_message_id": message_id, "force": force,
    })
    db.commit()
...
def _archive(contact_id: uuid.UUID, message: EmailMessage) -> None:
    ...
    try:
        name = f"{_now().strftime('%Y%m%dT%H%M%S')}-{contact_id}.eml"
